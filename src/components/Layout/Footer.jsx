...
export default function Footer() {
  ...
  const description = useMemo(() => {
    ...
    const pick =
      list.find((t) => t?.language_code === "en" || t?.language_id === 1) ||
      list[0];
    return pick?.value || "";
  }, [data]);
  ...
  const contactTranslation = useMemo(() => {
    const list = Array.isArray(primaryContact?.translations)
      ? primaryContact.translations
      : [];
    return (
      list.find((t) => t?.language_code === "en" || t?.language_id === 1) ||
      list[0] ||
  ...
  const phones = useMemo(() => {
    if (!primaryContact?.phones?.all)
      return primaryContact?.phones?.primary || "";
    return primaryContact.phones.all
      .map((p) => p.number)
      .filter(Boolean)
      .join(" | ");
  }, [primaryContact]);
      ...
      <footer className="footer-section-5 section-padding section-bg pb-0 pt-0">
                ...
                <div className="footer-widget-items">
                  ...
                  <ul className="list-area">
                    {usefulLinks.map((link) => {
                      const tList = Array.isArray(link?.translations)
                        ? link.translations
                        : [];
                      const t =
                        tList.find(
                          (x) =>
                            x?.language_code === "en" || x?.language_id === 1,
                        ) || tList[0];
                  ...
                  <ul className="contact-list">
                    ...
                    <li>
                      ...
                      <div className="content">
                        ...
                        <p>
                          <a
                            href={`tel:${primaryContact?.phones?.primary || ""}`}
                          >
                            {phones}
                          </a>
                        </p>
        ...
        <div className="footer-bottom" style={{ position: "relative" }}>
          <div className="container text-center">
            <div className="footer-bottom-wrapper">
              <p className="text-1">
                (c) 2026 <span>C-Dit</span>. All Rights Reserved
              </p>
            </div>
          </div>
        </div>
