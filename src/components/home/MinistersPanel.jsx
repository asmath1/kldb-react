...
import { useGetImportantPersonsQuery } from '../../redux/services/bannerApi'

export default function MinistersPanel() {
  const { data, isLoading, isError } = useGetImportantPersonsQuery()
  ...
  const ministers = useMemo(() => {
    ...
    return data.data
      ...
      .map((person) => {
        const translation = Array.isArray(person?.translations)
          ? person.translations.find((t) => t.language_code === 'en') || person.translations[0]
          : null

        return {
          id: person.id,
          img: `https://livestock.cditproject.org${person.photo_path}`,
          name: translation?.name || 'Unknown',
          role: translation?.bio || '',
        }
      })
  }, [data])

  if (isLoading) return <p>Loading ministers...</p>
  if (isError) return <p>Failed to load ministers.</p>
  if (!ministers.length) return <p>No ministers available.</p>
  return (
