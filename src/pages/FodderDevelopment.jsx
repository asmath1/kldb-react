...
export default function FodderDevelopment() {
  ...
  const currentSections = [...(current?.sections || [])].sort(
    (a, b) => a.order - b.order
  );
  ...
  return (
    <>
          ...
          <div className="blog-post-details border-wrap mt-0">
            <div className="single-blog-post post-details mt-0">
              <div className="post-content pt-0">
                ...
                {/* MAIN INTRO */}
                {mainT?.main_title && (
                  <h2 className="wow fadeInDown">{mainT.main_title}</h2>
                )}

                {mainT?.body && (
                  <div
                    className="wow fadeInUp fodder-intro"
                    dangerouslySetInnerHTML={{ __html: mainT.body }}
                  />
                )}
                    ...
                    <div className="infra-tabs">
                      {subArticles.map((art, i) => {
                        ...
                        return (
                          <button
                            key={art.id}
                            className={`infra-tab-btn ${
                              i === activeArticle ? "active" : ""
                            }`}
                            onClick={() => setActiveArticle(i)}
                          >
                            {at?.title || art.slug}
                    ...
                    <div className="fodder-tab-content mt-3">
                      ...
                      {currentSections.map((section) => {
                        ...
                        return (
                          <div
                            key={section.id}
                            className="fodder-section wow fadeInUp"
                          >
                            {/* TITLE */}
                            {st?.title && (
                              <h3 className="fodder-section-title">
                                {st.title}
                              </h3>
                            )}
      ...
      <style>{`
        ...
        .fodder-content table td,
        .fodder-content table th {
          padding: 8px 10px;
          border: 1px solid #ddd;
        }

        .fodder-content table tr:nth-child(even) td {
          background: #f9f9f9;
        }
